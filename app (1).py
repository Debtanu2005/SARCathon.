...
def get_LLM_response(query, option):
    ...
    elif option=="Curriculum and Faculty":
        examples= [
      {
    ...
    elif option=="Accreditation & Recognition":
        examples= [
      {
    ...
    elif option=="Tuition fee and Scholarships":
        examples= [
      {
    ...
    prefix = """You are a {template_option} agent of saras ai : 
    """
    ...
    new_prompt_template = FewShotPromptTemplate(
        ...
        prefix=prefix,
        suffix=suffix,
        input_variables=["template_userInput","template_option"],
        example_separator="\n"
    )
    if "sessionMessages" not in st.session_state:
        st.session_state.sessionMessages =[SystemMessage(content="You are a helpful assistant who gives clean and proper response as per the data provided in the fewshort template ")
                                      ]
    ...
    response= load_answer(new_prompt_template.format(template_userInput=query,template_option=option))
    print(response)

    return response


st.set_page_config(page_title="Herlper",
                   initial_sidebar_state='auto')

option=st.selectbox(
    'Please select the action to be performed?',
    ("Admissions","Curriculum and Faculty","Accreditation & Recognition","Career Services","Tuition fee and Scholarships")
)
...
if submit:
    st.write(get_LLM_response(question,option))
